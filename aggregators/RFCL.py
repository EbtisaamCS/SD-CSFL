...
class RFCLAggregator(Aggregator):
    """
    ...
    def generate_cluster_centres(self, models: List[nn.Module]) -> None:
        ...
        X = np.array(X)
        # Clean the data to replace NaNs with zeroes
        X = self.clean_data(X)
        pca = PCA.pca(X)
        self.clusterer = hdbscan.HDBSCAN(min_cluster_size=9, min_samples=6, allow_single_cluster=True).fit(pca) #16 ,14 #9,6 
    ...
    def _use_most_similar_clusters(self, K=2) -> Tuple[List[nn.Module], Tensor, List[int]]: #2
        """
        Uses Cosine similarity to determine the "most similar" clusters
        """
